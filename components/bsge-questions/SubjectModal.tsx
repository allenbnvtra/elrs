...
interface Props {
  editingSubject: { _id: string; name: string; description?: string } | null;
  form: SubjectFormData;
  saving: boolean;
  onClose: () => void;
  onSave: () => void;
  onChange: (form: SubjectFormData) => void;
}
...
/** Clamp a numeric string between 0 and max, stripping non-digits. */
...
export default function SubjectModal({ editingSubject, form, saving, onClose, onSave, onChange }: Props) {
  const set = (field: keyof SubjectFormData, value: string) =>
    onChange({ ...form, [field]: value });

  const totalSeconds =
    parseInt(form.timerHH || "0") * 3600 +
    parseInt(form.timerMM || "0") * 60 +
    parseInt(form.timerSS || "0");
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {totalSeconds > 0 && (
                <button
                  type="button"
                  onClick={() => onChange({ ...form, timerHH: "00", timerMM: "00", timerSS: "00" })}
