...
import {
  ...
  BookOpen, ArrowLeft, FileText, GraduationCap, Power, FileUp, AlertCircle,
  Clock,
} from "lucide-react";
import { useAuth } from "@/contexts/authContext";
import type { Subject, Question, Pagination, ImportResult } from "@/components/bsge-questions/types";
...
import SubjectModal, { SubjectFormData } from "@/components/bsge-questions/SubjectModal";
...
export default function BSGEQuestionsPage() {
  ...
  const [subjectForm, setSubjectForm] = useState<SubjectFormData>({
    name: "", description: "", timerHH: "00", timerMM: "00", timerSS: "00",
  });
  ...
  const openEditSubject = (s: Subject) => {
    ...
    setSubjectForm({
      ...
      timerHH: String(Math.floor(t / 3600)).padStart(2, "0"),
      timerMM: String(Math.floor((t % 3600) / 60)).padStart(2, "0"),
      timerSS: String(t % 60).padStart(2, "0"),
    });
    setShowSubjectModal(true);
  };
  ...
  const handleDownloadTemplate = async () => {
    ...
    try {
      ...
      a.href = url; a.download = "bsge_questions_template.xlsx"; a.click();
  ...
  const handleImportFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append("file", file); fd.append("userId", USER_ID); fd.append("course", "BSGE");
    ...
    <div className="space-y-6 animate-in fade-in duration-700">
      ...
      {showQuestionModal && USER_ID && (
        <QuestionModal
          ...
          onSaved={() => { fetchQuestions(); fetchSubjects(); }}
          userId={USER_ID}
      ...
      {showPDFImportModal && USER_ID && (
        <PDFImportModal
          ...
          onSuccess={() => { fetchQuestions(); fetchSubjects(); }}
          userId={USER_ID}
          defaultSubject={selectedSubject?.name}
        />
      )}
          ...
          {loadingSubjects ? (
            ...
            subjects.map((subject) => (
              <div key={subject._id} className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 hover:shadow-md transition-all group">
                ...
                {formatTimer(subject.timer) && (
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-[#7d1a1a]/8 rounded-lg mb-3">
                    <Clock size={11} className="text-[#7d1a1a]" />
                    <span className="text-[11px] font-bold text-[#7d1a1a]">{formatTimer(subject.timer)}</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  ...
                  <button onClick={() => handleViewSubject(subject)} className="text-xs font-bold text-[#7d1a1a] hover:underline">
                    View Questions →
                  </button>
                </div>
              </div>
