    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid grid-cols-3 gap-3 p-5 border-b border-gray-100">
          {[
            { label: "Imported", value: result.imported, cls: "emerald" },
            { label: "Duplicates", value: totalDups, cls: "amber" },
            { label: "Errors", value: result.errors.length, cls: "red" },
          ].map(({ label, value, cls }) => (
            <div key={label} className={`bg-${cls}-50 rounded-xl p-3 text-center`}>
              <p className={`text-2xl font-black text-${cls}-600`}>{value}</p>
              <p className={`text-[10px] font-bold text-${cls}-600 uppercase tracking-wide mt-0.5`}>{label}</p>
            </div>
          ))}
        </div>
