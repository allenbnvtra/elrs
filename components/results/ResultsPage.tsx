...
import {
  ...
  Download, RotateCcw, GraduationCap,
...
interface Stats {
  ...
  bsaben: { totalPool: number; correct: number; wrong: number; successRate: number };
  bsge:   { totalPool: number; correct: number; wrong: number; successRate: number };
}
...
export default function ResultsPage() {
  ...
  }, [user?.id, currentPage, selectedCourse, selectedResponse, searchQuery, isAdmin]);

  useEffect(() => { fetchStats(); },  [fetchStats]);
  useEffect(() => { fetchResults(); }, [fetchResults]);
  useEffect(() => { setCurrentPage(1); }, [selectedCourse, selectedResponse, searchQuery]);
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const a = Object.assign(document.createElement("a"), {
        ...
        download: `results-export-${Date.now()}.csv`,
      });
      a.click();
  ...
  const CourseStatCard = ({ course }: { course: "bsaben" | "bsge" }) => {
    const isBSABEN = course === "bsaben";
    const data     = stats![course];
    const accent   = isBSABEN ? "#7d1a1a" : "emerald-600";
    const accentCls = isBSABEN ? "text-[#7d1a1a]" : "text-emerald-600";
    const bgCls     = isBSABEN ? "bg-[#7d1a1a]/5" : "bg-emerald-50";
    const borderCls = isBSABEN ? "border-[#7d1a1a]" : "border-emerald-600";
    const iconColor = isBSABEN ? "text-[#7d1a1a]/10" : "text-emerald-600/10";
    const Icon      = isBSABEN ? GraduationCap : Book;
    const dept      = isBSABEN ? "Engineering Department" : "Geodetic Department";
    const label     = isBSABEN ? "BSABEN" : "BSGE";
    ...
    return (
      <div className="bg-white p-5 xs:p-7 sm:p-10 rounded-[24px] xs:rounded-[32px] sm:rounded-[40px] shadow-sm border border-gray-100 relative group overflow-hidden">
        <div className={`absolute top-0 right-0 p-4 xs:p-6 sm:p-8 ${iconColor} group-hover:scale-110 transition-transform`}>
          <Icon size={80} className="xs:w-[100px] xs:h-[100px] sm:w-[120px] sm:h-[120px]" />
        </div>
        <div className="relative z-10">
          <div className={`inline-flex items-center gap-1.5 xs:gap-2 px-2.5 xs:px-3 sm:px-4 py-1 xs:py-1.5 sm:py-2 ${bgCls} ${accentCls} rounded-full mb-3 xs:mb-4 sm:mb-6`}>
            <span className="text-[8px] xs:text-[9px] sm:text-[10px] font-black uppercase tracking-widest">{dept}</span>
          </div>
          <h3 className="text-xl xs:text-2xl sm:text-3xl font-black text-gray-900 mb-4 xs:mb-6 sm:mb-8 tracking-tight">
            {label} <span className={accentCls}>Stats</span>
          </h3>
          <div className="grid grid-cols-2 gap-4 xs:gap-6 sm:gap-10">
            {[
              { label: "Total Pool",    val: data.totalPool,    border: "border-gray-100",   color: "text-gray-400" },
              { label: "Success Rate",  val: `${data.successRate.toFixed(2)}%`, border: borderCls, color: accentCls },
              { label: "Correct",       val: data.correct,      border: "border-emerald-500", color: "text-emerald-500" },
              { label: "Wrong",         val: data.wrong,        border: "border-red-500",     color: "text-red-500" },
            ].map(({ label: l, val, border, color }) => (
              <div key={l} className={`border-l-2 xs:border-l-4 ${border} pl-2 xs:pl-3 sm:pl-4`}>
                <p className={`text-[8px] xs:text-[9px] sm:text-[10px] font-black ${color} uppercase tracking-widest mb-1 xs:mb-2`}>{l}</p>
    ...
    <div className="space-y-4 xs:space-y-6 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 xs:gap-4">
        <div>
          ...
          <h1 className="text-xl xs:text-2xl sm:text-3xl font-black text-gray-900 tracking-tight">Results</h1>
        </div>
        ...
        <div className="flex items-center gap-2 xs:gap-2.5 w-full sm:w-auto">
          ...
          <button className="flex items-center justify-center gap-1.5 xs:gap-2 bg-[#7d1a1a] text-white px-4 xs:px-5 sm:px-6 py-2.5 sm:py-3 rounded-lg xs:rounded-xl font-bold shadow-lg shadow-[#7d1a1a]/20 hover:bg-[#5a1313] transition-all active:scale-95 text-xs xs:text-sm flex-1 sm:flex-none">
            <Calendar size={16} />
            Generate Report
          </button>
        </div>
      ...
      {loadingStats ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 xs:gap-3 sm:gap-5">
          {[
            ...
            <div
              key={i}
              className={`bg-white p-3 xs:p-4 sm:p-6 rounded-2xl xs:rounded-[24px] sm:rounded-[32px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 flex flex-col items-start transition-all hover:-translate-y-1 ${i === 4 ? "col-span-2 md:col-span-1" : ""}`}
            >
              ...
              <p className="text-[8px] xs:text-[9px] sm:text-[10px] font-black text-gray-400 uppercase tracking-widest mb-0.5 xs:mb-1">
                {stat.label}
              </p>
      ...
      {/* COMPARATIVE STATS — admin sees both; faculty sees only their course */}
      {stats && (
        <div className={`grid grid-cols-1 gap-4 xs:gap-6 sm:gap-8 ${isAdmin ? "lg:grid-cols-2" : ""}`}>
          {(isAdmin || (user as any)?.course === "BSABEN") && <CourseStatCard course="bsaben" />}
          {(isAdmin || (user as any)?.course === "BSGE")   && <CourseStatCard course="bsge" />}
        </div>
      )}
      ...
      <div className="bg-white/80 backdrop-blur-xl p-4 xs:p-6 sm:p-8 rounded-[24px] xs:rounded-[32px] sm:rounded-[40px] border border-gray-200 shadow-xl">
        <div className="flex items-center gap-2 xs:gap-3 sm:gap-4 mb-4 xs:mb-5 sm:mb-6">
          ...
          <h4 className="font-black text-gray-900 text-[10px] xs:text-xs sm:text-sm uppercase tracking-widest">
            Filter Records
          </h4>
        </div>

        <div className={`grid grid-cols-1 gap-2 xs:gap-3 sm:gap-4 ${isAdmin ? "md:grid-cols-2 lg:grid-cols-4" : "md:grid-cols-3"}`}>
          {/* Search */}
      ...
      <div className="bg-white rounded-[24px] xs:rounded-[32px] sm:rounded-[48px] border border-gray-100 shadow-2xl overflow-hidden min-h-[400px] xs:min-h-[500px] sm:min-h-[600px]">
        <div className="p-4 xs:p-6 sm:p-10 border-b border-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 xs:gap-4">
          <div>
            <h3 className="text-lg xs:text-xl sm:text-2xl font-black text-gray-900 tracking-tight">Detailed Breakdown</h3>
            <p className="text-[9px] xs:text-[10px] sm:text-[11px] text-gray-400 font-bold uppercase tracking-[0.2em] mt-0.5 xs:mt-1">
            ...
            {(["card", "table"] as const).map((m) => (
              <button
                ...
                className={`flex-1 sm:flex-none px-4 xs:px-5 sm:px-6 py-1.5 xs:py-2 rounded-lg xs:rounded-xl text-[9px] xs:text-[10px] font-black uppercase transition-all ${
                  viewMode === m ? "bg-white text-gray-900 shadow-sm" : "text-gray-400"
                }`}
              >
                {m === "card" ? "Card View" : "Table View"}
              </button>
            ))}
          </div>
        </div>
                  ...
                  >
                    <div className="flex items-center gap-3 xs:gap-4 sm:gap-5 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[8px] xs:text-[9px] sm:text-[10px] text-gray-400 font-bold uppercase tracking-widest truncate">
                          {shortQuestionId(result.questionId)}
                        </p>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <span className={`block px-2 xs:px-3 sm:px-4 py-1 xs:py-1.5 sm:py-2 rounded-lg xs:rounded-xl text-[7px] xs:text-[8px] sm:text-[9px] font-black uppercase tracking-widest ${
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr className="text-[10px] xs:text-[11px] font-black text-gray-400 uppercase tracking-widest">
                    {["Student", "Question", "Subject", "Answer", "Status", "Date"].map((h) => (
                  ...
                  {results.map((result) => (
                    <tr key={result._id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        {result.studentName && <p className="text-xs text-gray-500">{result.studentName}</p>}
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-xs text-gray-600 line-clamp-2">{result.questionText}</p>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded">{result.subject}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <span className={`font-bold ${result.isCorrect ? "text-emerald-600" : "text-red-600"}`}>{result.selectedAnswer}</span>
                      ...
                      <td className="px-6 py-4 text-xs text-gray-500">{formatDate(result.submittedAt)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="p-4 xs:p-6 sm:p-10 border-t border-gray-50 flex flex-col xs:flex-row items-center justify-between gap-3 xs:gap-4">
            <button
              ...
              className="text-[9px] xs:text-[10px] font-black text-[#7d1a1a] uppercase tracking-[0.2em] flex items-center gap-1.5 xs:gap-2 hover:underline disabled:opacity-50 disabled:no-underline"
            >
