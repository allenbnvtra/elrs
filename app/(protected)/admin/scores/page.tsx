...
import { 
  ...
  XCircle, AlertCircle, Calendar, BookOpen, X
} from "lucide-react";
...
export default function ScoresPage() {
  ...
  const filteredScores = scores.filter(s => {
    const matchesCourse = s.course === selectedCourse;
  ...
  // Calculate stats
  const avgScore = filteredScores.reduce((acc, s) => acc + s.averageScore, 0) / filteredScores.length || 0;
  const passRate = (filteredScores.filter(s => s.averageScore >= 75).length / filteredScores.length * 100) || 0;
  const excellentCount = filteredScores.filter(s => s.status === "excellent").length;
      ...
      <div className="hidden lg:block bg-white rounded-[24px] border border-gray-200 shadow-sm overflow-hidden flex-col min-h-[500px]">
        <div className="flex-1 overflow-auto custom-scrollbar">
          <table className="w-full text-left border-collapse relative">
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredScores.length > 0 ? filteredScores.map((student, i) => {
        ...
        <div className="p-5 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
            Showing {filteredScores.length} of {scores.length} Students
          </p>
          <div className="flex items-center gap-2">
            <button 
            ...
            >
              ...
              <div className="mb-3 xs:mb-4">
                <div className="flex items-center justify-between mb-1.5 xs:mb-2">
                  ...
                  <span className="text-sm xs:text-base font-black text-gray-900">{student.averageScore}%</span>
                </div>
