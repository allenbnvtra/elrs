...
export default function ScoresPage() {
  ...
  const [selectedCourse, setSelectedCourse] = useState<"BSABEN" | "BSGE">(
    facultyCourse ?? "BSABEN"
  );
  ...
  }, [selectedCourse, currentPage, filterStatus, searchQuery, user?.id]);
  ...
  const handleExportCSV = async () => {
    ...
    setExporting(true);
    try {
      const params = new URLSearchParams({ course: selectedCourse, userId: user.id });
      if (filterStatus !== "all") params.set("status", filterStatus);
      if (searchQuery)            params.set("search", searchQuery);
  ...
  return (
    <div className="space-y-4 xs:space-y-5 sm:space-y-6 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 xs:gap-4">
        ...
        >
          {exporting
            ...
            : <Download size={16} />}
          <span>{exporting ? "Exporting..." : "Export Grades"}</span>
        </button>
      </div>
      ...
      <div className="flex flex-col gap-3 xs:gap-4 bg-white p-3 xs:p-4 rounded-xl xs:rounded-2xl border border-gray-200/60 shadow-sm">
        {/* Course tabs — hidden for faculty (locked to their course) */}
        {!isFaculty && (
      ...
      <div className="hidden lg:block bg-white rounded-[24px] border border-gray-200 shadow-sm overflow-hidden min-h-[500px]">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 z-10 bg-gray-50/90 backdrop-blur-md border-b border-gray-200">
              <tr className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                ...
                <th className="px-8 py-5 text-right">Last Exam</th>
              </tr>
            </thead>
                          ...
                          <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${
                                s.averageScore >= 90 ? "bg-emerald-500" :
                                s.averageScore >= 75 ? "bg-blue-500"    : "bg-amber-500"
                              }`}
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="p-5 border-t border-gray-100 bg-gray-50/30 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 text-gray-400 hover:bg-white border border-gray-200 rounded-lg disabled:opacity-30">
                <ChevronLeft size={18} />
              </button>
              {buildPageList().map((p, i) => (
                <button key={i} onClick={() => typeof p === "number" && setCurrentPage(p)} disabled={typeof p !== "number"}
                  className={`w-9 h-9 rounded-lg text-xs font-bold transition-colors ${p === currentPage ? "bg-[#7d1a1a] text-white" : typeof p === "number" ? "text-gray-600 hover:bg-gray-100" : "text-gray-400 cursor-default"}`}>
                  {p}
                </button>
              ))}
              <button onClick={() => setCurrentPage(p => Math.min(pagination.totalPages, p + 1))} disabled={currentPage === pagination.totalPages} className="p-2 text-gray-400 hover:bg-white border border-gray-200 rounded-lg disabled:opacity-30">
      ...
      <div className="lg:hidden space-y-3 xs:space-y-4">
        {loading ? (
          ...
          <div className="bg-white rounded-xl xs:rounded-2xl border-2 border-dashed border-gray-100 p-8 xs:p-12 flex flex-col items-center justify-center text-center">
            ...
            <p className="font-black uppercase tracking-widest text-xs xs:text-sm text-gray-500 mb-1">No Scores Found</p>
            <p className="text-[10px] xs:text-xs font-bold text-gray-400">Try adjusting your filters for {selectedCourse}</p>
          </div>
        )}
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="bg-white rounded-xl xs:rounded-2xl border border-gray-200 shadow-sm p-3 xs:p-4">
            ...
            <div className="flex items-center justify-center gap-1.5 xs:gap-2">
              <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-1.5 xs:p-2 text-gray-400 hover:bg-white border border-gray-200 rounded-lg disabled:opacity-30">
                <ChevronLeft size={16} />
              </button>
              {buildPageList().map((p, i) => (
                <button key={i} onClick={() => typeof p === "number" && setCurrentPage(p)} disabled={typeof p !== "number"}
                  className={`w-8 h-8 xs:w-9 xs:h-9 rounded-lg text-[10px] xs:text-xs font-bold ${p === currentPage ? "bg-[#7d1a1a] text-white" : typeof p === "number" ? "text-gray-600" : "text-gray-400"}`}>
                  {p}
                </button>
              ))}
              <button onClick={() => setCurrentPage(p => Math.min(pagination.totalPages, p + 1))} disabled={currentPage === pagination.totalPages} className="p-1.5 xs:p-2 text-gray-400 hover:bg-white border border-gray-200 rounded-lg disabled:opacity-30">
